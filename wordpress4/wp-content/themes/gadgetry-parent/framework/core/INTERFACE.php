...
class TF_INTERFACE extends TF_TFUSE
{
    ...
    public function admin_item_menu_page()
    {
        ...
        add_menu_page(__('ThemeFuse', 'tfuse'), $this->theme->theme_name, 'manage_options', 'themefuse', array($this, 'page_framework_options'), $tf_menu_icon, 27);
    ...
    public function cf_row_template($row = array(),$options_row=array())
    {
        ...
        foreach($row as $key => $option) {
            ...
            if($option['type'] == 'button') {
                ...
                $out .= '<div class="tfuse_button_subrow">' . $this->optigen->$call_method($option) . '</div>';
            }elseif($option['type'] == 'raw'){
                $call_method = $option['type'];
                $out .= '<div class="tfuse_subrow '.$option['id'].'">' . $this->optigen->$call_method($option) . '</div>';
            }
            else {
            if ($option['id'] == 'tf_cf_select[]'){
                ...
                $call_method = $option['type'];
                $out .= '<div class="tfuse_subrow '.$invisible.'">' . $this->optigen->$call_method($option) . '</div>';
            }
        }
        ...
        $out .= '<div class="tfclear divider"></div><ul '.$style.' class="ui-sortable sortable_options">';
                        ...
                        $out .= '<li class=" form-field tfuse-tax-form-field "><div class="corner_sdb"></div>';
                            foreach($option_row as $option){
                                $call_method = $option['type'];
                                $out .= '<div class="tfuse_subrow"><span>' . $option_type . '</span>' . $this->optigen->$call_method($option) . '<div class="tf_cf_delete_option"></div></div>';
