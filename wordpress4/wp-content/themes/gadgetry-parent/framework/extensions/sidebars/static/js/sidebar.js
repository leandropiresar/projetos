...
jQuery(document).ready(function(){
    ...
    $(document).on('keyup', '#add_new_sidebar_name', function(event){
        if(event.keyCode == 13){
            $(this).next('input[type="button"]').click();
        }
    });
    
    $(document).on('click', '#add_new_sidebar_submit', function(){
        $.post(ajaxurl,{
    ...
    $('.add_to_placeholder').click(function(){
        ...
        if( $(this).siblings('h2').length > 0 ) {
            /* for wp 4.4+ */
            this_name=$(this).siblings('h2').text();
        }
        else{
    ...
    $(document).on('change', '#extra_select', function(){
        $('#show_more_opts').hide();
    ...
    $(document).on('click', '.sidebars_placeholder_container', function(){
    ...
    $(document).on('click', '.sidebars_placeholder_box li .delete', function(){
    ...
    if(tf_script && tf_script.current_page && tf_script.current_page=='widgets') {
        ...
        $(document).on('click', '#tfuse_fields .sidebar-name', function(){
        ...
        $(document).on('mousedown','#tfusetab-sidebar_new_settings #extra_select',function(event){
        ...
        $(document).on('mousedown','#tfusetab-sidebar_new_settings .sidebars_subtype',function(event){
            if(sdb_has_changes==true) {
                if(!confirm(_unsaved_message)) {
                    event.preventDefault();
                    return false;
        ...
        $(document).on('sortstart', '.sidebars_placeholder_box', function(){
            if($(this).is(':visible'))
                sdb_changes_on();
        });
