...
class TF_CONTACTFORM extends TF_TFUSE
{
    ...
    function add_static()
    {
        ...
        foreach($options as $key=>$option){
            ...
            $this->include->$call_method( $option['filename'],$option['type'].$key,'tf_footer',($option['type'] == 'js') ? '20' : '' );
        }
    ...
    function add_menu()
    {
        add_menu_page(__('Contact Forms Settings', 'tfuse'), __("Contact Forms", 'tfuse'), 'publish_pages', 'tf_contact_forms_list', array($this, 'list_forms'), '', 28);
    ...
    function get_form_options()
    {
        ...
        foreach($options['tabs'] as $tab) {
                ...
                if($id !== false) {
                    ...
                    if($heading['id'] == 'form_settings') {
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= '</ul>
                            <input type="hidden" id="tf_form_main_content_id" data-form-content-id="'.sanitize_title($heading['name']).'" />';
                        $add_method = $add_button['type'];
                        $prev_method = $prev_button['type'];
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->optigen->$add_method($add_button);
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->optigen->$prev_method($prev_button);
                    }
                    foreach($heading['options'] as $option) {
            ...
            foreach($tab['buttons'] as $button) {
                ...
                $button_method = $button['type'];
                $admin_meta_boxes[$tab['id']]['buttons'] .= $this->optigen->$button_method($button);
            }
        }
