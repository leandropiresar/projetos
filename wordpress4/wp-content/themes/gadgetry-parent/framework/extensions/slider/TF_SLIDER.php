...
class TF_SLIDER extends TF_TFUSE {
    ...
    function add_menu() {
        if (count($this->general_options) <= 0)
            return;
        add_menu_page(apply_filters('tf_ext_slider_menu_label_page_title', __('Sliders', 'tfuse')), apply_filters('tf_ext_slider_menu_label_menu_title', __('Sliders', 'tfuse')), 'publish_pages', 'tf_slider_list', array($this, 'list_sliders'), '', 26);
