...
class TF_RESERVATIONFORM extends TF_TFUSE
{
    ...
    function __init()
    {
        ...
        require_once( TFUSE_THEME_DIR.'/framework/extensions/'.strtolower(
                $this->_the_class_name
            ).'/config/constants.php' );
        require_once( TFUSE_THEME_DIR.'/framework/extensions/'.strtolower(
                $this->_the_class_name
            ).'/config/utils.php' );
        if ($this->request->isset_GET( 'page' ) && $this->request->GET(
                'page'
            ) == 'tf_reservation_form' && $this->request->isset_GET( 'id' )
        ) {
            $this->redirect_if_id_invalid( $this->request->GET( 'id' ) );
        }
        add_action( 'admin_menu', array( $this, 'add_menu' ), 20 );
        $this->get->ext_config( 'RESERVATIONFORM', 'base' );
        if (is_admin() && $this->request->isset_GET( 'page' ) && stripos(
        ...
        if (file_exists( TEMPLATEPATH.'/theme_config/extensions/reservationform/options/preview_options.php' )) {
            ...
            if (@is_array( $options )) {
                foreach ($options as $key => $option) {
                    ...
                    $call_method = $option['type'];
                    $this->include->$call_method(
                        $option['filename'],
                        $option['type'].$key,
                        'tf_footer',
                        ( $option['type'] == 'js' ) ? '20' : ''
    ...
    {
        add_menu_page(
            __( 'Reservation Forms Settings', 'tfuse' ),
            apply_filters( 'res_form_top_menu', __( 'Reservations' ) ),
            'publish_pages',
            'tf_reservations_list',
            array( $this, 'list_reservations' ),
            '',
            29
        );
        ...
        add_submenu_page(
            'tf_reservations_list',
            apply_filters( 'res_form_res_list', __( 'User Reservations', 'tfuse' ) ),
            apply_filters( 'res_form_res_list', __( 'User Reservations', 'tfuse' ) ),
            'publish_pages',
            'tf_reservations_list',
            array( $this, 'list_reservations' )
        ...
        add_submenu_page(
            'tf_reservations_list',
            apply_filters( 'res_form_all_forms', __( 'All Reservation Forms', 'tfuse' ) ),
            apply_filters( 'res_form_all_forms', __( 'All Reservation Forms', 'tfuse' ) ),
            'publish_pages',
            'tf_reservation_forms_list',
            array( $this, 'list_forms' )
        );
        add_submenu_page(
            'tf_reservations_list',
            apply_filters( 'res_form_add_new', __( 'Add New Form', 'tfuse' ) ),
            apply_filters( 'res_form_add_new', __( 'Add New Form', 'tfuse' ) ),
            'publish_pages',
            'tf_reservation_form',
            array( $this, 'show_add_form' )
        );
        add_submenu_page(
            'tf_reservations_list',
            apply_filters( 'res_form_gen_set', __( 'General Settings', 'tfuse' ) ),
            apply_filters( 'res_form_gen_set', __( 'General Settings', 'tfuse' ) ),
            'publish_pages',
            'tf_reservation_forms_gensett',
            array( $this, 'list_gen_options' )
        );
    ...
    function list_reservations()
    {
        ...
        if ($this->request->isset_GET( 's' )) {
            ...
            $only_for_counting = new WP_Query(
                array_merge( $args, array( 'posts_per_page' => -1, 'post_status' => 'any' ) )
            );
        ...
        $array_statistic = array(
            'all'      => ( isset( $only_for_counting->found_posts ) ) ? $only_for_counting->found_posts : sizeof(
                $tmp_posts
            ),
            'pending'  => 0,
            'approved' => 0,
    ...
    {
        $child_path = TFUSE_CHILD_CONFIG.'/extensions/'.strtolower(
                $this->_the_class_name
            ).'/views/'.$name_file.'.php';
    ...
    function get_tc_view( $__name_file, $__data = null, $__return = false )
    {
        ...
        if ( ! $view_path) {
            exit( 'View not found View -> Theme Config: '.strtolower(
                    $this->_the_class_name
                ).'/views/'.$__name_file.'.php' );
        }
    ...
    function add_email_content()
    {
        ...
        if (trim( $message ) != '') {
            foreach ($the_form['input'] as $input) {
                $message = str_replace(
                    '['.$input["shortcode"].']',
                    $the_post[TF_THEME_PREFIX.'_'.$input["shortcode"]],
                    $message
                );
            }
        } else {
            foreach ($the_form['input'] as $input) {
                $value = ( in_array( $input['type'], array( 'date_in', 'date_out' ) ) ) ? date_i18n(
                    get_option( 'date_format' ),
                    strtotime( $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']] )
                ) : $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']];
        ...
        <a href="'.get_admin_url(
            ).'admin.php?page=tf_reservations_list" class="reject_selected_reservations button reset-button">Cancel</a>';
    ...
    function reservation_details()
    {
        ...
        $post_statuses = array(
            'publish' => __( 'Approved', 'tfuse' ),
            'draft'   => 'Rejected',
            'private' => __( 'Pending', 'tfuse' )
        );
        $options       = array(
            'tabs' => array(
                array(
        ...
        foreach ($the_form['input'] as $key => $input) {
            if ( ! isset( $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']] )) {
                $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']] = '-';
            }
            if ($input['type'] == 'res_email') {
                $html = '<a  class="res_mailto" href="mailto:'.urldecode(
                        $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']]
                    ).'">'.urldecode( $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']] ).'</a>';
            } else {
                $html = ( ! in_array(
                        $input['type'],
                        array( 'date_in', 'date_out' )
                    ) && $inputs[$input['type']]['name'] == 'Email' ) ? '<a  class="res_mailto" href="mailto:'.urldecode(
                        $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']]
                    ).'">'.urldecode( $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']] ).'</a>' : urldecode(
                    $the_post[TF_THEME_PREFIX.'_'.$input['shortcode']]
                );
            }
            $option          = array();
            $option['name']  = apply_filters(
                'res_input_name_'.TF_THEME_PREFIX.'_'.$input['shortcode'],
                __( urldecode( $input['label'] ), 'tfuse' )
            );
            $option['desc']  = '';
            $option['id']    = TF_THEME_PREFIX.'_'.$input['shortcode'];
            $option['type']  = 'raw';
            $option['html']  = '<div class="res_det_messages">'.( ( in_array(
                    $input['type'],
                    array( 'date_in', 'date_out' )
                ) ) ? date_i18n(
                    get_option( 'date_format' ),
        ...
        if (isset( $the_post['sent_messages'] )) {
            foreach ($the_post['sent_messages'] as $sent_message) {
                $option                                         = array();
                $option['name']                                 = date_i18n(
                    get_option( 'date_format' ),
                    strtotime( $sent_message['date'] )
                );
                $option['desc']                                 = __(
                    'Message sent by you to the user that made the reservation',
                    'tfuse'
                );
                $option['id']                                   = '';
                $option['type']                                 = 'raw';
                $option['html']                                 = '<div class="res_det_messages">'.urldecode(
        ...
        $reject_button  = ' <a id="tf_rf_reject_reservation" href="#" class="button reset-button">'.__(
                'Reject',
                'tfuse'
            ).'</a>';
        $message_button = ' <a id="tf_rf_send_message_reservation" href="#" class="button">'.__(
                'Send message',
                'tfuse'
            ).'</a>';
    ...
    function ajax_send_email( $ph_id = -1 )
    {
        ...
        $message  = ( $this->request->isset_POST( 'message' ) && trim(
        ...
        if (( $the_post == 'draft' && $status == -1 ) || ( $the_post == 'publish' && $status == 1 )) {
            return;
        }
        ...
        if (trim( $message ) != '') {
            if (isset( $the_form['input'] ) && ! empty( $the_form['input'] )) {
                foreach ($the_form['input'] as $input) {
                    $value   = ( in_array( $input['type'], array( 'date_in', 'date_out' ) ) ) ? date_i18n(
                        $date_format,
                        strtotime( $post[TF_THEME_PREFIX.'_'.$input['shortcode']] )
                    ) : $post[TF_THEME_PREFIX.'_'.$input['shortcode']];
        ...
        $mail_type                 = ( isset( $gen_sett['mail_type'] ) ) ? 'send'.ucwords(
                $gen_sett['mail_type']
            ) : 'sendWpmail';
        $the_form['email_subject'] = urlencode(
            str_replace( '[resnumber]', $nr, urldecode( $the_form['email_subject'] ) )
        );
        ...
        if ($this->request->isset_POST( 'post_id' )) {
            die;
        }
    }
    ...
    function delete_reservations()
    {
        ...
        if (is_array( $this->request->POST( 'resid' ) )) {
            ...
            wp_delete_post( $this->request->POST( 'resid' ), true );
        }
    }
    ...
    function list_gen_options()
    {
        $this->load->ext_view(
            $this->_the_class_name,
            'general_settings',
            array( 'ext_name' => $this->_the_class_name )
        );
    }
    ...
    function list_forms()
    {
        ...
        $this->load->ext_view(
            $this->_the_class_name,
            'list_forms',
            array( 'forms' => $forms, 'ext_name' => $this->_the_class_name )
        );
    ...
    {
        if ($this->request->isset_POST( 'save_form' ) || $this->request->isset_POST(
                'save_messages'
            ) || $this->request->isset_POST( 'save_dates' )
        ) {
            $this->save_form();
        }
        echo '<div style="clear:both;height:20px;">&nbsp;</div>';
    ...
    function ajax_datepicker_row()
    {
        ...
        echo json_encode(
            array(
                'html' => $this->interface->cf_row_template(
                    $this->date_in_out( 'date_out', $values, $this->request->POST( 'number' ) )
                )
            )
        );
        die;
    ...
    function submitFrontendForm()
    {
        ...
        $message           = '';
        $title             = ( trim( urldecode( $form['reservation_title'] ) ) != '' ) ? __(
            urldecode( $form['reservation_title'] ),
            'tfuse'
        ) : __( 'New Reservation', 'tfuse' );
        $post_content      = array();
        ...
        if ($status) {
            ...
            if (isset( $form['new_res_email_template'] ) && trim( $form['new_res_email_template'] ) != '') {
                ...
                foreach ($form['input'] as $input) {
                    if ($this->request->isset_POST( TF_THEME_PREFIX.'_'.$input["shortcode"] )) {
                        $message = str_replace( '['.$input["shortcode"].']',
                            urldecode( $this->request->POST( TF_THEME_PREFIX.'_'.$input["shortcode"] ) ),
                            $message
                        );
                    } else {
                ...
                foreach ($form['input'] as $input) {
                    $value = ( in_array( $input['type'], array( 'date_in', 'date_out' ) ) ) ? date_i18n(
                        get_option( 'date_format' ),
                        strtotime( $this->request->POST( TF_THEME_PREFIX.'_'.$input['shortcode'] ) )
                    ) : $this->request->POST( TF_THEME_PREFIX.'_'.$input['shortcode'] );
                    $message .= '<strong>'.urldecode( $input['label'] ).':</strong> '.urldecode($value).'<br />';
                }
            ...
            $form['email_subject'] = urlencode(
                str_replace( '[resnumber]', $nr, urldecode( $form['email_subject'] ) )
            );
            $mail_type             = ( $gen_sett['mail_type'] ) ? 'send'.ucwords(
                    $gen_sett['mail_type']
                ) : 'sendWpmail';
            ...
            $this->$mail_type(
                $gen_sett,
                $this->new_reservation_admin_email_content( $form, $post_content, $status ),
                $email,
                $form
            );
            ...
            echo json_encode( array( 'mess' => __( urldecode( $form['fail_mess'] ), 'tfuse' ), 'error' => true ) );
        }
        die;
    }
    ...
    function new_reservation_admin_email_content( $form, $post_content, $nr )
    {
        ...
        if (isset( $form['admin_email_template'] ) && $form['admin_email_template'] != '') {
            ...
            foreach ($post_content as $key => $value) {
                $content = str_replace( '['.tf_option_id_without_prefix( $key ).']', urldecode($value), $content );
            }
        } else {
            foreach ($form['input'] as $input) {
                if (isset( $post_content[TF_THEME_PREFIX.'_'.$input["shortcode"]] )) {
                    $content .= '<strong>'.__(
                            $input['label'],
                            'tfuse'
                        ).':</strong> '.urldecode($post_content[TF_THEME_PREFIX.'_'.$input["shortcode"]]).'<br />';
                }
            }
        }
    ...
    function delete_form()
    {
        ...
        if ( ! is_array( $form_IDs ) || ! sizeof( $form_IDs )) {
            die();
        }
    ...
    function save_form()
    {
        ...
        $form_to_save['submit_mess']            = ( trim( $form['tf_rf_mess_submit'] ) == '' ) ? 'Submit' : urlencode(
            @$form['tf_rf_mess_submit']
        );
        ...
        if ($this->input->is_ajax_request()) {
            ...
            wp_update_term(
                $form_id,
                'reservations',
                array( 'name' => $form_to_save['form_name'], 'description' => serialize( $form_to_save ) )
            );
            echo json_encode( array( 'saved' => 1 ) );
            die();
        } else {
            $new_term = wp_insert_term(
                $this->request->POST( 'tf_rf_formname_input' ),
                'reservations',
                array( 'description' => serialize( $form_to_save ) )
            );
            $pageURL  = 'http';
    ...
    function save_general_options()
    {
        if (wp_verify_nonce( $this->request->POST( 'res_form_gensett_nonce' ), 'res_form_gensett_nonce_action' )) {
            $gen_options = array(
                'mail_type'   => $this->request->POST( 'tf_rf_mail_type' ),
                'smtp_host'   => $this->request->POST( 'tf_rf_smtp_host' ),
                'smtp_user'   => $this->request->POST( 'tf_rf_smtp_user' ),
                'smtp_pwd'    => $this->request->POST( 'tf_rf_smtp_pwd' ),
                'smtp_port'   => $this->request->POST( 'tf_rf_smtp_port' ),
                'secure_conn' => $this->request->POST( 'tf_rf_secure_conn' )
            );
    ...
    function sendSmtp( $general_options, $message, $email, $form )
    {
        ...
        if ($this->request->isset_POST( TF_THEME_PREFIX.'_'.$from ) && filter_var(
                $this->request->POST( TF_THEME_PREFIX.'_'.$from ),
        ...
        foreach ($admin_meta_boxes as $tab => $box) {
            ...
            foreach ($box as $heading => $rows) {
                ...
                add_meta_box(
                    $boxid,
                    $heading,
                    array( $this, 'custom_admin_box_content' ),
                    $tab,
                    'normal',
                    'core',
                    $rows
    ...
    function get_form_options()
    {
        ...
        $add_button       = array(
            ...
            'desc'       => '',
            'id'         => 'tf_rf_add_new',
            'value'      => __( 'Add new field', 'tfuse' ),
            'type'       => 'button',
            'subtype'    => 'button',
            'properties' => array( 'class' => 'button rf_add_new' )
        );
        $prev_button      = array(
            'name'       => '',
            'desc'       => '',
            'id'         => 'tf_rf_prev_button',
            'value'      => __( 'Preview', 'tfuse' ),
            'type'       => 'button',
            'subtype'    => 'button',
            'properties' => array( 'class' => 'reset-button rf_form_preview button' )
        );
        $add_method = $add_button['type'];
        $prev_method = $prev_button['type'];
        ...
        foreach ($options['tabs'] as $tab) {
            if ($tab['id'] == 'tf_rf_general_settings') {
                continue;
            }
            ...
            foreach ($headings as $heading) {
                ...
                if ($id !== false) {
                    if ($heading['id'] != 'message_settings' && $heading['id'] != 'form_name' && $tab['id'] != 'tf_rf_dates_settings') {
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->interface->cf_row_template(
                                $form_labels
                            ).'<div class="tfclear divider" style="margin-bottom:0 !important;"></div>';
                    ...
                    if ( ! empty( $form->description['exclude_dates'] )) {
                        foreach ($form->description['exclude_dates'] as $exclude_date) {
                            if ($tab['id'] == 'tf_rf_messages_settings' || $tab['id'] == 'add_edit_forms') {
                                continue;
                            }
                    ...
                    foreach ($form->description['input'] as $key => $input) {
                        if ($tab['id'] == 'tf_rf_messages_settings' || $heading['id'] == 'form_name' || $tab['id'] == 'tf_rf_dates_settings') {
                            continue;
                        }
                        if (in_array( $input['type'], array( 'date_in', 'date_out' ) )) {
                            ...
                            $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->interface->cf_row_template(
                                $this->date_in_out( $input['type'], $values, $key )
                            );
                            continue;
                        }
                        if ($input['type'] == 'res_email') {
                        ...
                        foreach ($heading['options'] as $option) {
                            ...
                            if (sizeof( $form->description['input'] ) == 1) {
                                if ($option['type'] == 'delete_row') {
                                    $option['class'] = 'tf_rf_delete_row_last';
                                }
                            }
                            $row[] = $option;
                        }
                    ...
                    if ($heading['id'] == 'form_settings') {
                        ...
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->optigen->$add_method(
                            $add_button
                        );
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->optigen->$prev_method(
                            $prev_button
                        );
                    ...
                    foreach ($heading['options'] as $option) {
                        if ($heading['id'] == 'form_settings' || $tab['id'] == 'tf_rf_dates_settings' || ! is_array(
                                $option
                            )
                        ) {
                            continue;
                        ...
                        if ($option['id'] == 'tf_rf_new_res_email_template' && isset( $form->description['new_res_email_template'] )) {
                            $option['value'] = stripslashes(
                                urldecode( $form->description['new_res_email_template'] )
                            );
                        }
                        ...
                        if ($option['id'] == 'tf_rf_confirm_email_template') {
                            $option['value'] = stripslashes(
                                urldecode( $form->description['confirm_email_template'] )
                            );
                        }
                        if ($option['id'] == 'tf_rf_succ_mess') {
                            $option['value'] = stripslashes( urldecode( $form->description['succes_mess'] ) );
                        ...
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->interface->meta_box_row_template(
                            $option
                        );
                    ...
                    if ($heading['id'] == 'form_settings') {
                        foreach ($heading['options'] as $key => $option) {
                            ...
                            if ($option['id'] == 'tf_rf_input_options[]') {
                                ...
                                $heading['options'][$key]['properties']['class'] = str_replace(
                                    '%%visible%%',
                                    'hidden',
                                    $heading['options'][$key]['properties']['class']
                                );
                            }
                        ...
                        $values       = array(
                            'label'   => __( 'Check In', 'tfuse' ),
                            'width'   => '50',
                            'newline' => false
                        );
                        $email_values = array(
                            'label'   => __( 'Your Email', 'tfuse' ),
                            'width'   => '50',
                            'newline' => false
                        );
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->interface->cf_row_template(
                        ...
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->interface->cf_row_template(
                            $this->date_in_out( 'date_in', $values, 1 )
                        );
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->interface->cf_row_template(
                            $this->default_email( $email_values, 2 )
                        );
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= '</ul><input type="hidden" id="tf_form_main_content_id" data-form-content-id="'.sanitize_title(
                                $heading['name']
                            ).'" />';
                        $admin_meta_boxes[$tab['id']][$heading['name']] .= $this->optigen->$add_method(
                            $add_button
            ...
            if ( ! empty( $tab['buttons'] )) {
                foreach ($tab['buttons'] as $button) {
                    ...
                    $method = $button['type'];
                    $admin_meta_boxes[$tab['id']]['buttons'] .= $this->optigen->$method( $button );
                }
            }
        }
    ...
    function get_form_gen_options()
    {
        ...
        foreach ($options['tabs'] as $tab) {
            if ($tab['id'] == 'add_edit_forms' || $tab['id'] == 'tf_rf_messages_settings' || $tab['id'] == 'tf_rf_dates_settings') {
                continue;
            }
