...
class TF_NEWSLETTER extends TF_TFUSE {
    ...
    function __init()
    {
        if (is_admin())
            ...
            add_action( 'wp_print_scripts', array($this, 'add_static_clientside') );
        }
    ...
    public function add_static_clientside()
    {
        wp_enqueue_style('tfuse-newsletter-css', TFUSE_EXT_URI . '/' . strtolower($this->_the_class_name) . '/static/css/newsletter_clientside.css');
        wp_enqueue_script('tfuse-newsletter-css', TFUSE_EXT_URI . '/' . strtolower($this->_the_class_name) . '/static/js/newsletter_clientside.js');
    }
    ...
    function mailchimp() {
        $lists = array();
        if ($this->mc_api) {
            $number = apply_filters( 'tfuse_newsltter_malchimp_list_limit', 25 );
            $start = apply_filters( 'tfuse_newsltter_malchimp_list_start', 0 );
            $lists = $this->mc_api->lists( array(), $start, $number );
            $tmp = $lists['data'];
            $lists = array();
            foreach ($tmp as $list) {
