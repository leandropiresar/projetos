...
function tfuse_youtube($atts, $content = null)
{
    ...
    return '<div class="video"><iframe width="' . $width . '" height="' . $height . '" src="//www.youtube.com/embed/' . $video_id[0] . '?wmode=transparent" frameborder="0"></iframe></div>' . PHP_EOL;
}
...
function tfuse_vimeo($atts, $content)
{
    ...
    return '<div class="video"><iframe src="//player.vimeo.com/video/' . (int) substr(parse_url($link, PHP_URL_PATH), 1) . '?title=0&amp;byline=0&amp;portrait=0" width="' . $width . '" height="' . $height . '" frameborder="0"></iframe></div>' . PHP_EOL;
}
