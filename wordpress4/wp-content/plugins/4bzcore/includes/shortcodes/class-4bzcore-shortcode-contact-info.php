<?php
/**
 * Contact Info Shortcode Class
 *
 * @since 4bzCore 1.0.0
 *
 * @package 4bzCore 
 * @subpackage class-4bzcore-shortcode-contact-info.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Contact_Info' ) ) {  
	class FourBzCore_Shortcode_Contact_Info {
		/**
		...
		public function __construct() {
			$this->id_base = 'fourbzcore-contact-info-';
		}
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/contact-info.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/contact-info.php';
			}
