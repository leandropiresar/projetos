<?php
/**
 * Slideshow Shortcode Class
 *
 * A simple flexslider slideshow.
 *
 * @since 4bzCore 1.0.0
 *
 * @package 4bzCore 
 * @subpackage class-4bzcore-shortcode-slideshow.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Slideshow' ) ) {  
	class FourBzCore_Shortcode_Slideshow {
		/**
		...
		public function __construct() {
			$this->id_base = 'fourbzcore-slideshow-';
		}
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/slideshow.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/slideshow.php';
			}
