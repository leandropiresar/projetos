<?php
/**
 * Author Bio Shortcode Class
 *
 * @since 4bzCore 1.0.0
 *
 * @package 4bzCore
 * @subpackage class-4bzcore-shortcode-author-bio.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Author_Bio' ) ) {  
	class FourBzCore_Shortcode_Author_Bio {
		// Class Variables
		/**
		...
		public function __construct() {
			$this->id_base = 'fourbzcore-author-bio-';
		}
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/author-bio.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/author-bio.php';
			}
