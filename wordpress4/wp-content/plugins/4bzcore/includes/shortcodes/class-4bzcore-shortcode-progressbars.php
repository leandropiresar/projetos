<?php
/**
 * Progressbars Shortcode Class
 *
 * @since 4bzCore 1.0.0
 *
 * @package 4bzCore 
 * @subpackage class-4bzcore-shortcode-progressbars.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Progressbars' ) ) {  
	class FourBzCore_Shortcode_Progressbars {
		/**
		...
		public function __construct() {
			$this->id_base = 'fourbzcore-progressbars-';
		}
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/progressbars.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/progressbars.php';
			}
