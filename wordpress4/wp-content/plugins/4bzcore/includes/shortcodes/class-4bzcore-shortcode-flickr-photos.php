<?php
/**
 * Flickr Shortcode Class
 *
 * @since 4bzCore 1.0.0
 *
 * @package 4bzCore 
 * @subpackage class-4bzcore-shortcode-flickr-photos.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Flickr_Photos' ) ) {  
	class FourBzCore_Shortcode_Flickr_Photos {
		/**
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/flickr-photos.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/flickr-photos.php';
			}
