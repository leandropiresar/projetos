 ...
 *
 * @package 4bzCore
 * @subpackage class-4bzcore-shortcode-column.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Column' ) ) {  
	class FourBzCore_Shortcode_Column {
		// Class Variables
		/**
		...
		public function __construct() {
			$this->id_base = 'fourbzcore-column-';
		}
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/column.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/column.php';
			}
