<?php
/**
 * Facebook Comments Shortcode Class
 * 
 * @since 4bzCore 1.0.0
 *
 * @package 4bzCore 
 * @subpackage class-4bzcore-shortcode-facebook-comments.php
 */
...
if ( ! class_exists( 'FourBzCore_Shortcode_Facebook_Comments' ) ) {  
	class FourBzCore_Shortcode_Facebook_Comments {
		/**
		...
		public function __construct() {
			$this->id_base = 'fourbzcore-fb-comments-';
		}
		...
		public function display( $atts ) {
			...
			// Get the template
			$template = locate_template( 'templates/facebook-comments.php', false, false );
			
			// If theme does not provide a template, then will use the one provided by the plugin.
			if ( empty( $template ) ) {
				$template = dirname( dirname ( dirname(__FILE__) ) ) . '/templates/facebook-comments.php';
			}
